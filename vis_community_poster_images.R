...
### recreate features_among_selected_community.png using new data.
...
comm_feat %>% summarise_at(vars(fx_assign:other), funs(weighted.mean(., n_mem))) -> avg_comm_feat

cal_dist <- function(i, feat, avg) {
    target <- select(feat[i,], fx_assign:other)
    bind_rows(target, avg) %>% as.matrix %>% dist -> res
    res[1]
}


comm_feat %>% 
    bind_rows(bind_cols(tibble(comm_name = "Average"), 
                                  avg_comm_feat)) %>% 
    mutate(dist = c(map_dbl(1:nrow(comm_feat), cal_dist, 
                            feat = comm_feat, avg = avg_comm_feat), 0)
)%>%
    select(comm_name, fx_opencurly, fx_integer, fx_infix, fx_assign, dist, n_mem) -> selected_comm_feat
...
selected_comm_feat %>% 
    ...
    scale_fill_brewer(palette = 'Dark2') -> g_features

ggsave("visualization_community/features_among_community2.png", plot = g_features, width = 12, height = 6, units = "in", bg = "transparent")

mu_dist <- weighted.mean(selected_comm_feat$dist[], selected_comm_feat$n_mem)
sigma_dist <- sum(wt * (df_dist$dist - mu_dist)^2) %>% sqrt()
 
selected_comm_feat %>% filter(comm_name != "Average") %>% 
    summarise(mu_dist = weighted.mean(dist, n_mem), 
              sigma_dist = sqrt(sum((n_mem / sum(n_mem))* (dist - mu_dist)^2))) -> dist_summ
...
node_info %>% filter(cran_wc %in% comm_lab$community_ids) %>% 
    ...
    arrange(desc(n)) %>% rename("comm_name" = cols) %>% select(comm_name, top_pkg) -> top_pkg

comm_feat %>% left_join(top_pkg, by = "comm_name") %>% filter(comm_id %in% c(18, 25, 36, 35)) %>%
    ...
    mutate(long_name = fct_relevel(long_name, 
           ...
           percentage = proportion * 100)) -> network_supple_data
...
plot_target <- function(target_community = "Java", network_supple_data) {
    network_supple_data %>% filter(comm_name == target_community) %>% rowwise %>%
        ...
        head(1) %>% pull(subtitle) -> subtitle
    network_supple_data %>% filter(comm_name == target_community) %>%
        mutate(long_name = fct_relevel(long_name, 
                                   "dotted.func", "ALLUPPER", "UpperCamel", "other", "alllower", 
                                   "lowerCamel", "lower_snake")) %>%
...
map(c("Java", "Machine learning", "Social Network Analysis", "Text Analysis"), 
