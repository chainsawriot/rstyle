...
saveRDS(tarballs, fs::path("data", 'pkg_meta.RDS'))

### Re-runing the extraction for those failed packages
pkg_meta <- readRDS(fs::path("data", "pkg_meta.RDS"))
...
saveRDS(final_meta, fs::path("data", "final_meta.RDS"))
...
final_meta <- readRDS(fs::path("data", "final_meta.RDS"))
set.seed(46709394)
